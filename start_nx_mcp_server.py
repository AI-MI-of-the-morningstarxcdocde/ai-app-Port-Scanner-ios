...
PID_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nx_mcp_server.pid")
...
def start_daemon():
    ...
    log_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nx_mcp_server.log")
...
def check_if_port_in_use(port):
    ...
    try:
        ...
        pid = subprocess.check_output(find_cmd, shell=True, text=True).strip()
...
""")
        ...
        with open(os.path.join(script_dir, "node_server.pid"), "w") as pid_file:
